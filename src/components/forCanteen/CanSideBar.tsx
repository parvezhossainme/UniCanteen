...
export default function CanSideBar() {
    ...
    const sidebarClasses = cn(
        "border-r border-orange-200/50 dark:border-orange-900/50 transition-all duration-300 ease-in-out transform hidden sm:flex h-full",
        "bg-gradient-to-b from-orange-50/90 to-amber-50/90 dark:from-slate-900/90 dark:to-slate-800/90 backdrop-blur-md shadow-lg",
    ...
    return (
        <div className="pr-4 h-full">
            <div className={sidebarClasses}>
                <aside className="flex h-full flex-col w-full px-4 overflow-hidden">
                    <div className="flex-1 overflow-y-auto scrollbar-hide">
                        {navSections.map((section, sIdx) => (
                            <div key={sIdx} className="mb-4">
                                <div className="text-xs font-bold text-orange-700 dark:text-orange-400 uppercase mb-2 mt-6 tracking-widest h-4">
                                    ...
                                    {section.items.map((item, idx) => (
                                        <Fragment key={idx}>
                                            <SideNavItem
                                                ...
                                                icon={item.icon}
                                                path={item.href}
                                                active={isClient ? item.active : false}
